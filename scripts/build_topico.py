...
def pick_item_icon(text: str) -> str:
    t = clean_md_title(text).lower()
    rules = [
        (("promotor", "piloto", "decid"), "fa-gavel"),
        (("motor", "desempenho", "velocidade"), "fa-rocket"),
        (("curadoria", "combust", "insumo"), "fa-bolt"),
        ...
        if "|" in line and i + 1 < len(lines):
            ...
            if (
                ...
                for h in header_cells:
                    table_html.append(f"<th>{inline_md(h)}</th>")
                table_html.append("</tr>")
                table_html.append("</thead>")
                ...
                for r in norm_rows:
                    table_html.append("<tr>")
                    for c in r:
                        table_html.append(f"<td>{inline_md(c)}</td>")
                    table_html.append("</tr>")
                table_html.append("</tbody>")
                table_html.append("</table>")
...
def render_menu(cards: List[Card], card_icons: List[str], menu_icon: str) -> str:
    links = []
    for idx, card in enumerate(cards, start=1):
...
def render_menu_from_labels(cards: List[Card], card_icons: List[str], labels: list[str], menu_icon: str) -> str:
    links = []
    for idx, card in enumerate(cards, start=1):
        base_label = strip_leading_number(card.title)
        short = f"{idx}. {strip_leading_number(base_label)}"
        icon = card_icons[idx - 1] if idx - 1 < len(card_icons) else pick_icon(short, menu_icon)
        links.append(
            f'            <a href="#p{idx}" class="nav-l"><i class="fas {icon}"></i> {esc(short)}</a>'
        )
...
def apply_global_page_rules(html_out: str, out_path: Path, page_title: str, menu_group_title: str) -> str:
    ...
    # Regra 2: Logo sempre vem de logo_base64.txt na pasta do arquivo de saída.
    logo_file = out_path.parent / "logo_base64.txt"
    if not logo_file.exists():
        raise FileNotFoundError(f"Arquivo obrigatório não encontrado: {logo_file}")
    logo_data_uri = normalize_data_uri(logo_file.read_text(encoding="utf-8"))
...
def main() -> None:
    ...
    menu_group_title, menu_labels = parse_menu_md(menu_md_path)
    menu_html = render_menu_from_labels(cards, card_icons, menu_labels, args.menu_icon)
